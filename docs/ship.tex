...
\subsubsection{Comm}
The Comm system is used for ship-to-ship communications.  

In an emergency, the distress call button can be used by clicking on the
distress button, which will then illuminate the button and call for help.
...
Open Items:
\begin{itemize}
\item How does the Comm work?  Do we want to do something similar to the
original SDFL?  Do we want to use buttons?
...
\item Do we want the ship's radio to be controller through here or some
other way (key combinations?)
\end{itemize}
